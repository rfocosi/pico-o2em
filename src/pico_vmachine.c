...
static uint8_t snapedlines[MAXLINES+2*MAXSNAP][16][2];
...
{
	int i;
	uint8_t reg_idx = reg & 0x0F;
	if (pos<MAXLINES+MAXSNAP+MAXSNAP)
   {
		for (i=0; i<mxsnap; i++)
      {
			if (snapedlines[pos+MAXSNAP-i][reg_idx][t]) return pos-i;
			if (snapedlines[pos+MAXSNAP+i][reg_idx][t]) return pos+i;
		}
		snapedlines[pos+MAXSNAP][reg_idx][t]=1;
	}
	return pos;
}
